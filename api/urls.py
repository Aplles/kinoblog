...
urlpatterns = [
    ...
    path('posts/<int:id>/', PostDetailUpdateDestroyView.as_view()),
    # Детальный просмотр(get), редактирование(path),удаление(delete)
    path('users/auth/', UserLoginView.as_view()),
    path('users/register/', UserCreateView.as_view()),
    path('posts/<int:id>/comments/', CommentListView.as_view(), name="create_comment"),  # Создание комментариев(post)
]
